...
_CONFIDENCE_RULES = """For every ConfidenceField:
  - value: the extracted value.
  - confidence: "confirmed" when the value is explicitly stated.
  - confidence: "inferred" only when the paper clearly implies the value without
    spelling it out, and explain the inference in evidence or warning.
  - confidence: "assumed" only for non-numeric, low-risk conventions explicitly
    marked as assumptions. Do not assume numeric hyperparameters.
  - source: section/table/paragraph identifier if available, otherwise a short
    phrase like "provided excerpt".
  - evidence: the shortest supporting phrase from the text.
"""
...
Rules:
  - Preserve DatasetDescription as plain values, not ConfidenceField objects.
...
class ExtractionAgent:
    ...
    def _prefer_confidence_field(
        ...
        if rank.get(new_value.confidence.value, 0) > rank.get(old_value.confidence.value, 0):
            return new_value
        return old_value
